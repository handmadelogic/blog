  ...
  return (
    <section className="relative w-full px-6 md:px-[90px] pt-16 md:pt-[80px] pb-16 md:pb-[88px] font-nunito text-[rgb(42,24,37)]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-10 mb-14">
        ...
        {/* Featured article */}
        <Link href="/posts/knitting-a-sweater" className="relative z-10">
        <article data-card-lift="true" className="relative cursor-pointer" style={{transform: 'matrix(0.999848, -0.0174524, 0.0174524, 0.999848, 0, 0)'}}>
          <p data-handwrite="true" className="hidden md:block absolute -top-2 -right-12 z-10 text-[rgb(201,52,126)] text-[22px] font-semibold font-caveat" style={{transform: 'matrix(0.997564, -0.0697565, 0.0697565, 0.997564, 0, 0)'}}>← my favorite this month ♡</p>
      ...
      {/* Bottom row: varied-width columns so no card reads identically */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1.35fr_1fr_0.82fr] gap-9">
        {[
          {
            href: '#',
          ...
          },
          {
            ...
            title: 'Watercolour florals (with unsolicited help).',
            desc: "Small hands, big opinions. I called it 'collaborative'.",
            date: 'Mar 22', mins: 5,
          ...
          <Link key={card.title} href={card.href} className={card.annotation ? 'relative z-10' : undefined}>
          <article data-card-lift="true" className="relative cursor-pointer h-full" style={{transform: card.rotate}}>
            {card.annotation && (
              <p data-handwrite="true" data-handwrite-speed="fast" className="hidden md:block absolute -top-2 -right-12 z-10 text-[rgb(122,148,104)] text-[22px] font-semibold font-caveat" style={{transform: 'matrix(0.997564, -0.0697565, 0.0697565, 0.997564, 0, 0)'}}>
                {card.annotation}
              </p>
            )}
            <div className="bg-[rgb(255,250,235)] rounded-[12px] p-4 h-full" style={{boxShadow: 'rgba(42, 24, 37, 0.1) 0px 10px 30px 0px, rgba(42, 24, 37, 0.05) 0px 2px 6px 0px'}}>
              <div className={`relative w-full h-[180px] sm:h-[200px] ${card.imgH} overflow-hidden rounded-lg`} style={{backgroundImage: card.gradient}}>
                <div className="absolute inset-0" style={{backgroundImage: 'radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.25), rgba(0, 0, 0, 0) 60%)'}} />
                <div className="absolute top-3 left-3 bg-[rgb(246,240,220)] text-[11px] font-bold tracking-[0.44px] px-3 py-1 rounded-full" style={{color: card.categoryColor}}>{card.category}</div>
              </div>
              <div className="pt-4 px-2 pb-2">
